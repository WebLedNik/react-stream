...
const Line: React.FC<LineProps> = (props) => {
  ...
  const updateTransformingPart = (props: updateTransformingPartProps) => {
    ...
    const updatedTargetPart: Part = {
      ...
      start: (isWithinIntervalValidateIntervalPrevPart || isWithinIntervalValidateIntervalNextPart ) ?
        (isWithinIntervalValidateIntervalPrevPart ? validateIntervalPrevPart.end : (targetPart.orientation === Orientation.Horizontal ? {x: targetPart.start.x, y: validateIntervalNextPart.start.y} : {x: validateIntervalNextPart.start.x, y: targetPart.start.y})): {
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    if (!lastPart) return createPart({line, orientation: Orientation.Horizontal})
  ...
  const handlePathMouseDown = (event: React.MouseEvent, part: Part) => {
    event.stopPropagation()
    //console.log('handlePathMouseDown', {line})
    const payload: LineState = {
      ...line,
      updating: true
    }
    console.log('handlePathMouseDown', {part})
  ...
  return (
    <g className={'flowchart-editor_line'}>
      <UpdatingPath parts={parts}/>
      <Path onClick={handlePathClick} onMouseDown={handlePathMouseDown} parts={line.parts} selected={line.selected}/>
      <circle
        data-id={line.id}
